import { Meta, Story, Canvas } from '@storybook/addon-docs';

import solution from './findDiagonalOrder.py?file';
import code from './findDiagonalOrder.py?raw';
import json from '../testCases.json?file';
import testCases from './testCases.py?file';

export const c0 = 'cases[0]';

<Meta title="Data Structure/Array/2D/Medium/[498] Diagonal Traverse/Solution/Python" />

# Solution

Modify the solution file `./findDiagonalOrder.py` to test results.

## Default Case

<Canvas withSource="open">
  <Story name="Case 0" parameters={{ docs: { source: { code } } }} inline={false} height="130px">
    <py-env>
    {`
      - paths:
        - ${json}
        - ${testCases}
        - ${solution}
    `}
    </py-env>
    <div>
      <div>Input: </div>
      <py-script>
      {`
        from testCases import cases
        print(${c0}['input'])
      `}
      </py-script>
    </div>
    <br />
    <div>
      <div>Output: </div>
      <py-script>
      {`
        from testCases import cases, runTestCase
        from findDiagonalOrder import Solution
        solution = Solution()
        print(runTestCase(solution.findDiagonalOrder, ${c0}))
      `}
      </py-script>
    </div>
  </Story>
</Canvas>

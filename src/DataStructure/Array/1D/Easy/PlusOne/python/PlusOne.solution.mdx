import { Meta, Story, Canvas } from '@storybook/addon-docs';

import solution from './plusOne.py?file';
import code from './plusOne.py?raw';
import json from '../testCases.json?file';
import testCases from './testCases.py?file';

export const c0 = 'cases[0]';

<Meta title="Data Structure/Array/1D/Easy/[66] Plus One/Solution/Python" />

# Solution

Modify the solution file `./plusOne.py` to test results.

## Default Case

<Canvas withSource="open">
  <Story name="Case 0" parameters={{ docs: { source: { code } } }} inline={false} height="130px">
    <py-env>
    {`
      - paths:
        - ${json}
        - ${testCases}
        - ${solution}
    `}
    </py-env>
    <div>
      <div>Input: </div>
      <py-script>
      {`
        from testCases import cases
        print(${c0}['input'])
      `}
      </py-script>
    </div>
    <br />
    <div>
      <div>Output: </div>
      <py-script>
      {`
        from testCases import cases, runTestCase
        from plusOne import Solution
        solution = Solution()
        print(runTestCase(solution.plusOne, ${c0}))
      `}
      </py-script>
    </div>
  </Story>
</Canvas>

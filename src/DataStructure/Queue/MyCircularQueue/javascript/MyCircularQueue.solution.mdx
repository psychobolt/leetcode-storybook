...
import code from './MyCircularQueue.es3.cjs?raw';
import runTestCase, { cases } from './testCases.js';

export const c0 = cases[2];
export const printOutput = memoize(() => JSON.stringify(runTestCase(solution, cloneDeep(c0))));

<Meta title="Data Structure/Queue/[622] Design Circular Queue/Solution/Javascript" />

# Solution

Modify the solution file `./MyCircularQueue.es3.cjs` to test results.
...
<Canvas withSource="open">
  <Story name="Case 0" parameters={{ docs: { source: { code } } }}>
    ...
    <div>
      <div>Args: </div>
      <pre>{JSON.stringify(c0.args)}</pre>
    </div>
    <br />
    <div>
      <div>Output: </div>
